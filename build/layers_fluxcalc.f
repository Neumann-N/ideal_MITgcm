...
C     layers_diagFreq ::
     ...
     &                       layers_bolus
      ...
      Real*8 layers_taveFreq, layers_diagFreq
...
C      MapFact  :: factors for interpolating T(Z) to T(ZZ)
      ...
      Real*8 layers_bounds(Nlayers+1,layers_maxNum)
      Real*8 dZZf(FineGridMax)
      INTEGER MapIndex(FineGridMax), CellIndex(FineGridMax)
      Real*8 MapFact(FineGridMax)
      INTEGER NZZ
...
C      iLa  :: layer coordinate index
      ...
      Real*8 UH     (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nlayers,nSx,nSy)
      Real*8 Hw     (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nlayers,nSx,nSy)
      Real*8 PIw    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nlayers,nSx,nSy)
      Real*8 Uw     (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nlayers,nSx,nSy)
      Real*8 VH     (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nlayers,nSx,nSy)
      Real*8 Hs     (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nlayers,nSx,nSy)
      Real*8 PIs    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nlayers,nSx,nSy)
      Real*8 Vs     (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nlayers,nSx,nSy)
      INTEGER iLa
CEOP
...
C     Tloc,Tp1 :: horizontally interpolated tracer values
      ...
      Real*8 TatU(sNx+1,sNy+1), TatV(sNx+1,sNy+1)
      Real*8 dzfac
      INTEGER kcip1
      Real*8 delPsi, maskp1
      LOGICAL errorFlag
...
C     log2(Nlayers)) as log2(Nlayers) + 1 for safety
      mSteps = int(log10(dble(Nlayers))/log10(2.D0))+1
...
C     Reset the arrays
      DO kg=1,Nlayers
       DO j = 1-OLy,sNy+OLy
        DO i = 1-OLx,sNx+OLx
         UH (i,j,kg,bi,bj) = 0.D0
         Hw(i,j,kg,bi,bj) = 0.D0
         PIw(i,j,kg,bi,bj) = 0.D0
         Uw(i,j,kg,bi,bj) = 0.D0
         VH (i,j,kg,bi,bj) = 0.D0
         Hs(i,j,kg,bi,bj) = 0.D0
         PIs(i,j,kg,bi,bj) = 0.D0
         Vs(i,j,kg,bi,bj) = 0.D0
        ENDDO
       ENDDO
      ENDDO
...
C ------ Find theta at the U point (west) on the fine Z grid
         ...
         TatU(i,j) = MapFact(kk) *
     &    0.5D0 * (tracer(i-1,j,k,bi,bj)+tracer(i,j,k,bi,bj)) +
     &    (1.D0 -MapFact(kk)) *
     &    0.5D0 * (tracer(i-1,j,kp1,bi,bj)+tracer(i,j,kp1,bi,bj))
...
C ------ Augment the bin values
     ...
     &      + delPsi*recip_drF(kci)*recip_hFacW(i,j,kci,bi,bj)
     &      * dzfac
         ENDIF
...
C ------ Find theta at the V point (south) on the fine Z grid
         ...
         TatV(i,j) = MapFact(kk) *
     &    0.5D0 * (tracer(i,j-1,k,bi,bj)+tracer(i,j,k,bi,bj)) +
     &    (1.D0 -MapFact(kk)) *
     &    0.5D0 * (tracer(i,j-1,kp1,bi,bj)+tracer(i,j,kp1,bi,bj))
...
C ------ Augment the bin values
     ...
     &      + delPsi*recip_drF(kci)*recip_hFacS(i,j,kci,bi,bj)
     &      * dzfac
         ENDIF
...
C     PIw       :: 1 if layer exists, 0 otherwise (at tracer point)
      INTEGER iLa, myThid
      Real*8 tracer    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,     nSx,nSy)
      Real*8 Hc        (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nlayers,nSx,nSy)
      Real*8 PIc       (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nlayers,nSx,nSy)
      Real*8 TtendSurf (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nlayers-1,nSx,nSy)
      Real*8 TtendDiffh(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nlayers-1,nSx,nSy)
      Real*8 TtendDiffr(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nlayers-1,nSx,nSy)
      Real*8 TtendAdvh (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nlayers-1,nSx,nSy)
      Real*8 TtendAdvr (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nlayers-1,nSx,nSy)
      Real*8 Ttendtot  (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nlayers-1,nSx,nSy)
      Real*8 StendSurf (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nlayers-1,nSx,nSy)
      Real*8 StendDiffh(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nlayers-1,nSx,nSy)
      Real*8 StendDiffr(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nlayers-1,nSx,nSy)
      Real*8 StendAdvh (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nlayers-1,nSx,nSy)
      Real*8 StendAdvr (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nlayers-1,nSx,nSy)
      Real*8 Stendtot  (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nlayers-1,nSx,nSy)
CEOP
