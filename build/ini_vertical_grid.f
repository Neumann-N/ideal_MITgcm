...
C        rF(K) = -H*(1.d0-dtanh(lambda*(NZ-dk      ))/dtanh(lambda*NZ))
C        rC(K) = -H*(1.d0-dtanh(lambda*(NZ-dk-0.5d0))/dtanh(lambda*NZ))

        delR(K) = lambda*H 
     &            / (dcosh(lambda*(NZ-dk-0.5d0))**2*dtanh(lambda*NZ))
C        drC(K) = lambda*H
C     &            / (dcosh(lambda*(NZ-dk      ))**2*dtanh(lambda*NZ))
      ENDDO
      
C       rF(Nr+1) = -H
c     There is no drC(Nr+1)
c      drC(Nr+1) = lambda*H/dtanh(lambda*NZ)
CLW)
